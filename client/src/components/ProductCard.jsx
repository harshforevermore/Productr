...
const ProductCard = ({ product }) => {
  ...
  const { togglePublish, deleteProduct } = useProducts();
    ...
    <>
      <div className="product-card w-[363px] bg-white rounded-2xl p-4 shadow-[0px_2px_4px_0px_#00000010] flex flex-col gap-4 border border-gray-100 font-['Inter']">
        ...
        <div className="details-container flex flex-col gap-1">
          ...
          <div className="specs text-[13px] leading-6 text-[#6B7280] flex flex-col gap-0.5">
            <div className="flex justify-between">
              ...
              <span className="text-[#344054]">{product.productType}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#98A2B3]">Quantity Stock -</span>
              <span className="text-[#344054]">{product.quantity}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-[#344054]">{product.brandName}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="actions flex items-center gap-2 mt-2">
          ...
          <button onClick={() => setShowUpdateProductModal(true)} className="cursor-pointer flex-1 h-9 px-2 rounded-lg border border-[#D4D4D4] text-md font-medium text-[#344054] hover:border-[#344054] flex items-center justify-center gap-2 group">
            Edit
          </button>

          <button
            onClick={() => setShowDeleteModal(true)}
            className="delete-button cursor-pointer w-9 h-9 rounded-md border border-[#D4D4D4] hover:border-[#344054] flex items-center justify-center transition-colors"
          >
            <svg
              width="16"
              height="21"
